module.exports = function() {
  ...
  myDescribe('チェック', function() {
    ...
    // emptyDir
    {
      ...
      it('emptyDir() でディレクトリが空の状態で存在する事を保証できる', async function() {
        ...
        const worked = await entries.emptyDir(src);
      ...
      it('ensureDirSync() でディレクトリが空の状態で存在する事を同期的に保証できる', function() {
        ...
        const worked = entries.emptyDirSync(src);
    ...
    // ensureFile
    {
      ...
      it('ensureFile() でファイルの存在を保証できる', async function() {
        const entries = await fq.load(src);
        const worked = await entries.ensureFile(name);
        assert.strictEqual(worked.path, dest);
        assert.strictEqual(worked.isFile, true);
      });
      it('ensureFileSync() でファイルの存在を同期的に保証できる', function() {
        const entries = fq.sync(src);
        const worked = entries.ensureFileSync(name);
        assert.strictEqual(worked.path, dest);
        assert.strictEqual(worked.isFile, true);
      });
    }
  ...
  myDescribe('文字コード', { data: 2 }, function() {
    ...
    it('テキスト', async function() {
      const entries = await fq.load(target);
      const encoding = await entries.detectEncoding();
      assert.strictEqual(encoding, 'Shift-JIS');
    });

    it('テキスト 同期', function() {
      const entries = fq.sync(target);
      const encoding = entries.detectEncodingSync();
      assert.strictEqual(encoding, 'Shift-JIS');
    });
  });
}